...
public class Level3Panel extends JPanel implements KeyListener {
    ...
    // Slowdown: increase delay per tile move over time
    private long moveCooldownMs = 120;
    private long lastMoveTime = 0L;
    private javax.swing.Timer slowTimer;

    private int lastDx = 0;
    private int lastDy = -1;

    public Level3Panel(IceCreamGUI parent, String sabor) {
        this.parent = parent;
        ...
        // Every 15s, increase movement delay by 60ms up to 600ms
        slowTimer = new javax.swing.Timer(15000, e -> {
            moveCooldownMs = Math.min(moveCooldownMs + 60, 600);
        });
        slowTimer.start();
    }
    ...
    private void drawScorePanel(Graphics g) {
        ...
        g.drawString("SCORE L3", panelX + 70, panelY + 35);
    ...
    public void keyPressed(KeyEvent e) {
        ...
        if (dxTile != 0 || dyTile != 0) {
            ...
            if (now - lastMoveTime < moveCooldownMs) {
                return; // movement delayed by Narval's slowing effect
            }
            lastMoveTime = now;
            lastDx = dxTile;
            lastDy = dyTile;
        ...
        if (code == KeyEvent.VK_SPACE) placeIceInLastDirection();
        if (code == KeyEvent.VK_Q) breakIceInLastDirection();
        ...
        frutas.removeIf(f -> {
            if (f.checkCollision(helado.getX(), helado.getY())) {
                ...
                if (score >= 16) {
                    stopAllTimers();
                    Object[] opciones = {"Reiniciar Nivel 3", "Terminar"};
                    ...
                    if (opcion == 0) parent.showPanel(new Level3Panel(parent, saborInicial));
